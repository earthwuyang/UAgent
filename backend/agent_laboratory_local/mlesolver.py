        ...
        try:
            ...
            for _ln in lines_to_replace:
                if _ln < len(current_code):
                    ...
                    print(f"Warning: Attempted to pop line {_ln} but code only has {len(current_code)} lines")
            for _line in lines_to_add:
                if args[0] <= len(current_code):
                    current_code.insert(args[0], _line)
                else:
                    print(f"Warning: Attempted to insert at line {args[0]} but code only has {len(current_code)} lines")
                    current_code.append(_line)  # Append instead
            new_code = "\n".join(current_code)
            code_exec = f"{args[4]}\n{new_code}"
            code_ret = execute_code(code_exec)
            if "CODE EXECUTION ERROR" in code_ret: return (False, None, code_ret)
...
class MLESolver:
    ...
    def solve(self):
        ...
        if len(self.best_codes) > 0 and top_score > self.best_codes[-1][1]:
