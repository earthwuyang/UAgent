...
class LaboratoryWorkflow:
    def __init__(self, research_topic, openai_api_key=None, dashscope_api_key=None, max_steps=100, num_papers_lit_review=5, agent_model_backbone=f"{DEFAULT_LLM_BACKBONE}", notes=list(), human_in_loop_flag=None, compile_pdf=True, mlesolver_max_steps=3, papersolver_max_steps=5, paper_index=0, except_if_fail=False, parallelized=False, lab_dir=None, lab_index=0, agentRxiv=False, agentrxiv_papers=5):
        ...
        self.api_key = dashscope_api_key if dashscope_api_key else openai_api_key
        ...
        self.save = True
        self.verbose = True
        self.reviewers = ReviewersAgent(model=self.model_backbone, notes=self.notes, openai_api_key=self.api_key)
        self.phd = PhDStudentAgent(model=self.model_backbone, notes=self.notes, max_steps=self.max_steps, openai_api_key=self.api_key)
        self.postdoc = PostdocAgent(model=self.model_backbone, notes=self.notes, max_steps=self.max_steps, openai_api_key=self.api_key)
        self.professor = ProfessorAgent(model=self.model_backbone, notes=self.notes, max_steps=self.max_steps, openai_api_key=self.api_key)
        self.ml_engineer = MLEngineerAgent(model=self.model_backbone, notes=self.notes, max_steps=self.max_steps, openai_api_key=self.api_key)
        self.sw_engineer = SWEngineerAgent(model=self.model_backbone, notes=self.notes, max_steps=self.max_steps, openai_api_key=self.api_key)
    ...
    def perform_research(self):
        ...
        @return: None
        """
        # Import supervision utility for cost monitoring (with higher threshold for less aggressive stopping)
        try:
            from supervision import LabSupervisor
            supervisor = LabSupervisor(self.lab_dir if self.lab_dir else ".", max_cost_threshold=50.0)
            if self.verbose: print("🔍 Supervision enabled - monitoring for dummy implementations and cost overruns")
        except ImportError:
            supervisor = None
            if self.verbose: print("⚠️  Supervision not available - proceeding without monitoring")
        ...
        for phase, subtasks in self.phases:
            ...
            for subtask in subtasks:
                ...
                # Supervision check after each subtask
                if supervisor:
                    should_stop, stop_reason = supervisor.should_stop_execution()
                    if should_stop:
                        print(f"🛑 SUPERVISION STOP: {stop_reason}")
                        print("💰 To prevent further costs, stopping execution.")
                        if self.verbose:
...
def parse_yaml(yaml_file_loc):
    ...
    if 'dashscope-api-key' in agentlab_data: parser.dashscope_api_key = agentlab_data["dashscope-api-key"]
...
if __name__ == "__main__":
    ...
    dashscope_api_key = (os.getenv('DASHSCOPE_API_KEY') or args.dashscope_api_key) if (hasattr(args, 'dashscope_api_key') or os.getenv('DASHSCOPE_API_KEY')) else None
    ...
    if dashscope_api_key is not None and os.getenv('DASHSCOPE_API_KEY') is None: os.environ["DASHSCOPE_API_KEY"] = args.dashscope_api_key

    if not api_key and not deepseek_api_key and not dashscope_api_key: raise ValueError("API key must be provided via --api-key / --deepseek-api-key / --dashscope-api-key or the OPENAI_API_KEY / DEEPSEEK_API_KEY / DASHSCOPE_API_KEY environment variable.")
