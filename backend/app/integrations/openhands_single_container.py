...
Create the directory experiments/{cfg.session_name}/results/ if needed. This final.json file is MANDATORY."""
        ...
        # Add proxy detection and usage instructions
        enhanced_goal += "\n\nPROXY CONFIGURATION:\n- ALWAYS check if a proxy is available on localhost:7890 before making network requests (git clone, wget, curl, etc.)\n- Test proxy availability: curl -x http://127.0.0.1:7890 http://www.google.com -I - If proxy is detected, configure it for all network operations:\n  * For git: git config --global http.proxy http://127.0.0.1:7890 && git config --global https.proxy http://127.0.0.1:7890\n  * For wget: export http_proxy=http://127.0.0.1:7890 && export https_proxy=http://127.0.0.1:7890\n  * For curl: use -x http://127.0.0.1:7890 flag\n  * For pip: pip install --proxy http://127.0.0.1:7890\n- If proxy test fails, proceed without proxy\n- Use proxy for ALL network operations including: git clone, wget, curl, pip install, npm install, etc.\n"
