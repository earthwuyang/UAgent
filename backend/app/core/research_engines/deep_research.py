...
"""
        ...
        try:
            plan_response = await self.llm_client.generate(plan_prompt, max_tokens=get_max_tokens_from_env(), temperature=0.4)  # Use environment variable for max tokens
...
"""
        ...
        try:
            reflection_response = await self.llm_client.generate(reflection_prompt, max_tokens=get_max_tokens_from_env(), temperature=0.5)  # Use environment variable for max tokens
    ...
    async def _synthesize_results(
        ...
        try:
            response = await self.llm_client.generate(synthesis_prompt, max_tokens=get_max_tokens_from_env(), temperature=0.4)  # Use environment variable for max tokens
