...
class CodeActRunnerV2:
    ...
    Provides: ensure_bootstrap, create_if_absent, write, read, run (with repetition control).
    """
    ...
    async def ensure_bootstrap(self) -> Dict[str, Any]:
        """Ensure venv exists and requirements (if present) are installed. Idempotent.
        ...
        try:
            ...
            pass
        # create venv
        jid = await self.client.start_job("python3 -m venv ./.venv && ./.venv/bin/pip install -U pip wheel", timeout_sec=900)
    ...
    async def run(self, cmd: str, timeout_sec: int = 300) -> ActionResult:
        k = hashlib.sha256(cmd.encode("utf-8")).hexdigest()
        c = self._cmd_counts.get(k, 0)
        if c >= 2:
            return ActionResult(id="repeat-skipped", tool="run", success=True, exit_code=0, duration_ms=1, stdout="skipped repeated command")
        self._cmd_counts[k] = c + 1
