...
class AgentManager:
    ...
    def _get_task_desc_str(self):
        from ..anti_fraud_prompts import get_enhanced_system_prompt
...
"""
        ...
        # Enhance with anti-fraud requirements
        research_domain = self.task_desc.get("Name", "").lower()

        # Detect if native compilation is requested from task description
        constraints = {}
        full_desc = (self.task_desc.get("Title", "") + " " +
                    self.task_desc.get("Abstract", "") + " " +
                    self.task_desc.get("Short Hypothesis", "")).lower()

        if any(keyword in full_desc for keyword in ['clone', 'compile', 'source code', 'native', 'build from source', 'make']):
            # Check if Docker is explicitly NOT mentioned
            if 'docker' not in full_desc and 'container' not in full_desc:
                constraints['native_compilation'] = True

        task_desc = get_enhanced_system_prompt(task_desc, research_domain, constraints)
        task_desc += (
            "Title:\n"
