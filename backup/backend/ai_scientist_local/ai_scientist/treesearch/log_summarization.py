...
client, model = create_client("qwen-max")
...
def overall_summarize(journals):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        # Handle variable number of results
        # The function expects exactly 4 results: draft, baseline, research, ablation
        # If we have more or fewer results, we need to map them correctly
        if len(results) >= 4:
            draft_summary = results[0] if len(results) > 0 else {}
            baseline_summary = results[1] if len(results) > 1 else {}
            research_summary = results[2] if len(results) > 2 else {}
            ablation_summary = results[3] if len(results) > 3 else {}
        else:
            # Pad with empty dicts if we have fewer than 4 results
            draft_summary = results[0] if len(results) > 0 else {}
            baseline_summary = results[1] if len(results) > 1 else {}
            research_summary = results[2] if len(results) > 2 else {}
            ablation_summary = {}
