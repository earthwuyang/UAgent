...
def perform_experiments_bfts(config_path: str):
    ...
    # 🔍 CRITICAL: Validate experiments for fraud before processing results
    print("\n🔍 Validating experiments for scientific integrity...")
    import sys
    import os
    sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
    from experiment_validator import ExperimentValidator

    validator = ExperimentValidator(str(cfg.log_dir.parent))
    validation_report = validator.validate_experiment()

    if validation_report["status"] == "FAILED":
        print("❌ EXPERIMENT VALIDATION FAILED - RESULTS INVALIDATED")
        print("🚨 CRITICAL VIOLATIONS DETECTED:")
        for violation in validation_report["violations"][:10]:  # Show first 10
            print(f"   • {violation}")
        print(f"\n📋 Full validation report: {cfg.log_dir.parent}/validation_report.json")
        print("\n🛑 EXPERIMENT TERMINATED DUE TO SCIENTIFIC INTEGRITY VIOLATIONS")
        raise ValueError("Experiment validation failed due to scientific integrity violations")
    elif validation_report["status"] == "WARNING":
        print("⚠️  EXPERIMENT VALIDATION PASSED WITH WARNINGS")
        print("⚠️  Please review warnings in validation report")
    else:
        print("✅ EXPERIMENT VALIDATION PASSED - Results are scientifically valid")
