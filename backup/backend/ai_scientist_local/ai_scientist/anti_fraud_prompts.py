...
def _sanitize_docker_references(text: str) -> str:
    """Remove or replace Docker-specific references when native compilation is required"""
    # Replace specific Docker command patterns
    text = text.replace("FROM ubuntu:22.04", "# Use appropriate build environment")
    text = text.replace("dockerfile = '''FROM ubuntu:22.04", "# dockerfile = '''# Use native compilation")
    text = text.replace("dockerfile_content = '''FROM ubuntu:22.04", "# dockerfile_content = '''# Use native build")

    # Replace Docker terminology (case-insensitive)
    import re
    text = re.sub(r'\bDocker\b', "Native build environment", text, flags=re.IGNORECASE)
    text = re.sub(r'\bdocker\b', "native", text)
    text = re.sub(r'\bcontainer\b', "local environment", text, flags=re.IGNORECASE)
    text = re.sub(r'\bContainer\b', "Local environment", text)
    text = re.sub(r'\bDockerfile\b', "build script", text, flags=re.IGNORECASE)

    # Replace Docker-specific commands
    text = re.sub(r'docker\s+run', 'native execution', text)
    text = re.sub(r'docker\s+build', 'native build', text)
    text = re.sub(r'docker-compose', 'local setup', text)
...
NATIVE_COMPILATION_REQUIREMENTS = """
🔧🚨 NATIVE COMPILATION ENVIRONMENT IS REQUIRED 🚨🔧

CRITICAL: You are building software from source code natively on the host system.
NO Docker containers should be used. Build directly on the system.

🔴 NO USE OF:
- Docker or docker-compose commands
- Container-based builds
- Dockerfiles or Docker images

🟢 NATIVE BUILD REQUIREMENTS:
1. Use system package managers (apt, yum, brew) to install dependencies
2. Clone source code directly with git
3. Use native build tools: make, cmake, configure, autotools
4. Install libraries directly on the host system
5. Run executables directly without containerization

EXAMPLE NATIVE BUILD SETUP:
```python
import subprocess
import os
...
def build_from_source():
    ...
    subprocess.run(['sudo', 'apt-get', 'install', '-y', 'build-essential', 'git', 'libreadline-dev', 'zlib1g-dev'])

    # Clone and build from source
    subprocess.run(['git', 'clone', 'https://github.com/postgres/postgres.git'])
    os.chdir('postgres')
...
✅ DO use git clone for source code
✅ DO use native compilation tools
✅ DO install dependencies on host system
✅ DO run make, configure, cmake directly
✅ DO use absolute paths for installation
❌ DON'T use Docker or containers
❌ DON'T create Dockerfiles
❌ DON'T use docker build or docker run commands
"""

def get_enhanced_system_prompt(original_prompt: str, research_domain: str = "general", constraints: dict = None) -> str:
    """
    Enhance any system prompt with anti-fraud requirements
    """
    # Check if native compilation is explicitly requested via constraints
    if constraints and (constraints.get('no_docker') or constraints.get('native_compilation')):
        # Use native compilation requirements instead of Docker
        enhanced_prompt = original_prompt + "\n\n" + NATIVE_COMPILATION_REQUIREMENTS
    else:
        # Always sanitize Docker template examples to prevent accidental Docker generation
        # when native compilation keywords are detected in the task description
        docker_requirements = DOCKER_MANDATORY_REQUIREMENTS

        # Check if task contains native compilation keywords
        task_desc_lower = original_prompt.lower()
        if any(keyword in task_desc_lower for keyword in ['clone', 'compile', 'source code', 'native', 'build from source', 'make']):
            # Remove Docker template examples that could bias toward Docker
            docker_requirements = docker_requirements.replace("FROM ubuntu:22.04", "# Use appropriate build environment")
            docker_requirements = docker_requirements.replace("dockerfile = '''FROM ubuntu:22.04", "# dockerfile = '''# Use native compilation")
            docker_requirements = docker_requirements.replace("dockerfile_content = '''FROM ubuntu:22.04", "# dockerfile_content = '''# Use native build")

        enhanced_prompt = original_prompt + "\n\n" + docker_requirements

    # Add general anti-fraud requirements (sanitized if native compilation)
    real_exp_req = REAL_EXPERIMENTATION_REQUIREMENTS
    if constraints and (constraints.get('no_docker') or constraints.get('native_compilation')):
        real_exp_req = _sanitize_docker_references(real_exp_req)
    enhanced_prompt += "\n\n" + real_exp_req

    if "postgresql" in research_domain.lower() or "postgres" in research_domain.lower():
        postgres_req = POSTGRESQL_MODIFICATION_REQUIREMENTS
        if constraints and (constraints.get('no_docker') or constraints.get('native_compilation')):
            postgres_req = _sanitize_docker_references(postgres_req)
        enhanced_prompt += "\n\n" + postgres_req

    if "htap" in research_domain.lower() or "query" in research_domain.lower():
        htap_req = HTAP_RESEARCH_REQUIREMENTS
        if constraints and (constraints.get('no_docker') or constraints.get('native_compilation')):
            htap_req = _sanitize_docker_references(htap_req)
        enhanced_prompt += "\n\n" + htap_req

    # Always add implementation success guidance to prevent common failures
    impl_success = IMPLEMENTATION_SUCCESS_PROMPTS
    if constraints and (constraints.get('no_docker') or constraints.get('native_compilation')):
        impl_success = _sanitize_docker_references(impl_success)
    enhanced_prompt += "\n\n" + impl_success
