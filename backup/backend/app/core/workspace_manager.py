...
"""
    ...
    def _generate_deployment_instructions(self, workspace: Path, saved_files: Dict[str, List[str]], llm_output: str):
        ...
        deployment_method = self._detect_deployment_method_from_files(saved_files, llm_output)

        if deployment_method == "native":
            self._add_native_deployment_instructions(instructions, workspace, scripts, code_files, configs, has_script, script_cmd)
        elif deployment_method == "docker":
            self._add_docker_deployment_instructions(instructions, workspace, scripts, code_files, configs, has_script, script_cmd)
        else:
            self._add_generic_deployment_instructions(instructions, workspace, scripts, code_files, configs, has_script, script_cmd)
    ...
    def _detect_deployment_method_from_files(self, saved_files: Dict[str, List[str]], llm_output: str) -> str:
        """Detect deployment method from generated files and LLM output"""
        code_files = saved_files.get('code_files', [])
        configs = saved_files.get('configs', [])

        # Check for explicit Docker files
        has_docker_files = any('Dockerfile' in f or 'docker-compose' in f for f in code_files + configs)

        # Check for native build files
        has_native_files = any('Makefile' in f or 'configure' in f or 'CMakeLists' in f for f in code_files + configs)

        # Check LLM output for indicators
        llm_lower = llm_output.lower()
        docker_mentions = llm_lower.count('docker') + llm_lower.count('container')
        native_mentions = llm_lower.count('make') + llm_lower.count('compile') + llm_lower.count('gcc') + llm_lower.count('configure')

        # Decision logic
        if has_docker_files or docker_mentions > native_mentions:
            return "docker"
        elif has_native_files or native_mentions > 0:
            return "native"
        else:
            return "auto"

    def _add_native_deployment_instructions(self, instructions, workspace, scripts, code_files, configs, has_script, script_cmd):
        """Add native deployment instructions"""
        instructions.append("\n## Prerequisites")
        prerequisites = [
            "Build tools (make, gcc, g++)",
            "Development libraries for the target software",
        ...
        # Check for native build patterns
        has_makefile = any('Makefile' in f or 'makefile' in f for f in code_files)
        has_configure = any('configure' in f for f in code_files)
        ...
        if has_script('install.sh'):
            instructions.append(script_cmd('install.sh'))
        elif has_makefile:
            instructions.append("sudo make install")
        ...
        instructions.append("\n## Verification")
        verify_steps = [
            "ps aux | grep <service_name>  # Check if service is running",
            "systemctl status <service_name>  # Check systemd service status",
            "<service_name> --version  # Verify installation",
        ]
        instructions.append("```bash")
        ...
        instructions.append("\n## Troubleshooting")
        trouble = [
            "If compilation fails, ensure all development dependencies are installed",
            "For permission errors, run installation commands with appropriate privileges",
            "Check system logs with `journalctl -f` for runtime errors"
        ]
        for item in trouble:
            instructions.append(f"- {item}")

    def _add_docker_deployment_instructions(self, instructions, workspace, scripts, code_files, configs, has_script, script_cmd):
        ...
        if has_script('build.sh'):
            ...
            instructions.append("docker build -t custom-image .")
        ...
        verify_steps = [
            ...
            "docker exec -it <container_name> <command>  # Execute commands in container",
        ]
        instructions.append("```bash")
        instructions.extend(verify_steps)
        instructions.append("```")
        ...
        instructions.append("\n## Troubleshooting")
        trouble = [
            "If Docker build fails, check Dockerfile syntax and base image availability",
            "For container startup issues, examine logs with `docker logs <container>`",
            "For port conflicts, update port mappings in docker-compose.yml"
        ]
        for item in trouble:
            instructions.append(f"- {item}")

    def _add_generic_deployment_instructions(self, instructions, workspace, scripts, code_files, configs, has_script, script_cmd):
        """Add generic deployment instructions when method is unclear"""
        instructions.append("\n## Prerequisites")
        instructions.append("- Check generated files to determine specific requirements")
        ...
        instructions.append("\n## Troubleshooting")
        instructions.append("- Examine the generated files to understand the deployment approach")
