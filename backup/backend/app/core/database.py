    ...
    def save_research_goal(self, goal_id: str, goal: ResearchGoal) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Goal {goal_id} contains non-serializable data: {json_err}")
                logger.error(f"Success criteria: {goal.success_criteria}")
                logger.error(f"Constraints: {goal.constraints}")
                # Try to clean the constraints by converting enum objects to strings
                cleaned_constraints = {}
                if goal.constraints:
                    for key, value in goal.constraints.items():
                        try:
                            ...
                            if hasattr(value, 'value'):  # Enum object
                                cleaned_constraints[key] = value.value
                            else:
                                cleaned_constraints[key] = str(value)
                    goal.constraints = cleaned_constraints
                    logger.info(f"Cleaned constraints for goal {goal_id}: {goal.constraints}")
            ...
            logger.error(f"Goal data - Title: {goal.title}")
            logger.error(f"Goal data - Constraints type: {type(goal.constraints)}")
            if goal.constraints:
                for key, value in goal.constraints.items():
                    logger.error(f"Constraint {key}: {type(value)} = {value}")
            return False
