...
class NodeExecutionEngine:
    ...
    async def _execute_code_validation(
        ...
        insights = []
        if success:
            insights.append("Build and deployment validated successfully.")
        else:
    ...
    async def _attempt_auto_fix(
        ...
        prompt = f"""You are an expert DevOps engineer. A validation script failed during deployment.
...
Analyze the failure and output JSON with:
  ...
  "commands": optional shell commands to run after updating files (e.g., clean up processes, fix permissions).
  "notes": optional extra guidance for the next attempt.
Ensure that any port conflicts, missing files, or compilation issues are fixed by modifying the files accordingly.
"""
    ...
    async def _cleanup_containers(self, workspace: Path) -> None:
        # Clean up Docker containers if present
        ...
        # Clean up native processes if any (kill processes using ports from config)
        try:
            # Look for common service ports and kill processes using them
            common_ports = [5432, 3306, 8080, 8000, 9000]
