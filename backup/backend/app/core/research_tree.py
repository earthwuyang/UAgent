...
[
  {{
                ...
                if needs_verification:
                    ...
                    if workspace_path:
                        ...
                        if not verification_result.success:
                            ...
                            # Try autonomous deployment error fixing
                            deployment_fix_result = await self._attempt_deployment_error_fix(
                                workspace_path, task_description, verification_result.errors, config
                            )

                            if deployment_fix_result and deployment_fix_result['success']:
                                self._log_execution(node, "INFO", "🔧 LLM successfully fixed deployment errors")
                                ...
                                if retry_verification.success:
                                    ...
                                    execution_result.insights.insert(0, "🔧 Deployment errors fixed automatically by LLM")
                                    execution_result.insights.extend([
                                        "Infrastructure deployment errors resolved",
                                        f"Fixed issues: {deployment_fix_result['fixes_applied']}",
                                        "Re-verification successful"
                                    ])
                                    execution_result.metrics['deployment_errors_fixed'] = True
                                    execution_result.metrics['fixes_applied'] = deployment_fix_result['fixes_applied']
                                else:
    ...
    async def _attempt_deployment_error_fix(
        self, workspace_path: str, task_description: str, errors: list, config: dict
    ) -> Optional[Dict[str, Any]]:
        """Attempt to fix deployment errors using LLM analysis"""
        try:
            logger.info(f"🔧 Attempting autonomous deployment error fixing for {len(errors)} errors")

            # Detect deployment method from workspace
            deployment_files = self._read_deployment_files(workspace_path)
            deployment_method = self._detect_deployment_method_from_workspace(deployment_files)

            # Analyze error types and determine if they're fixable
            fixable_errors = self._classify_deployment_errors(errors, deployment_method)

            if not fixable_errors:
                logger.warning("No fixable deployment errors identified")
                return {'success': False, 'reason': 'No fixable errors identified'}

            if not deployment_files:
                logger.warning("No deployment files found to fix")
                return {'success': False, 'reason': 'No deployment files found'}

            # Use LLM to generate fixes based on deployment method
            fix_result = await self._generate_deployment_fixes(
                task_description, fixable_errors, deployment_files, config, deployment_method
            )
    ...
    def _classify_deployment_errors(self, errors: list, deployment_method: str) -> list:
    ...
    def _read_deployment_files(self, workspace_path: str) -> Dict[str, str]:
        """Read deployment-related files from workspace"""
        import os
        ...
        common_files = [
            ...
            # Native build files
            'Makefile', 'configure', 'CMakeLists.txt', 'build.sh', 'make.sh',
            # Config files
            'postgresql.conf', 'my.cnf', 'redis.conf',
            # Scripts
            'deploy.sh', 'run.sh', 'test.sh', 'install.sh',
            # Source files that might indicate compilation
            ...
            for filename in common_files:
                ...
                if os.path.exists(filepath):
                    with open(filepath, 'r') as f:
                        deployment_files[filename] = f.read()
        except Exception as e:
            logger.error(f"Error reading deployment files: {e}")

        return deployment_files

    async def _generate_deployment_fixes(
        self, task_description: str, fixable_errors: list, deployment_files: Dict[str, str], config: dict, deployment_method: str
    ) -> Optional[Dict[str, Any]]:
        """Use LLM to generate deployment configuration fixes"""
        ...
        if deployment_method == "docker":
            ...
            system_prompt = """You are a systems deployment expert. Analyze native compilation and deployment errors and provide fixes.

            Common fixes:
            1. Missing dependencies: Install required packages or libraries
            2. Compilation errors: Fix Makefile, configure scripts, or build commands
            3. Permission errors: Set correct file permissions or ownership
            4. Configuration errors: Fix config files, environment variables, or paths
            5. Port conflicts: Change default ports or service configurations

            Return a JSON object with fixes for each file:
            {
                "success": true,
                "fixes": {
                    "Makefile": "fixed content here",
                    "configure": "fixed script here",
                    "build.sh": "fixed script here",
                    "postgresql.conf": "fixed config here"
                }
            }"""
...
Native Deployment Errors:
{error_context}

Current Deployment Files:
{files_context}

Please fix these native compilation/deployment errors by providing corrected file contents. Focus on:
- Resolving compilation dependencies and build configuration
- Fixing Makefile, configure scripts, or build commands
- Setting correct permissions and file paths
- Resolving port conflicts in configuration files
...
Consider: Does this task generate executable code, infrastructure, or scripts that need to actually work?"""
    ...
    def _detect_deployment_method_from_workspace(self, deployment_files: Dict[str, str]) -> str:
        """Detect deployment method from workspace files"""
        if not deployment_files:
            return "auto"

        # Check for Docker files
        docker_files = ["Dockerfile", "docker-compose.yml", "docker-compose.yaml"]
        has_docker = any(filename in deployment_files for filename in docker_files)

        # Check for native build files
        native_files = ["Makefile", "configure", "CMakeLists.txt", "build.sh", "make.sh"]
        has_native = any(filename in deployment_files for filename in native_files)

        if has_docker and not has_native:
            return "docker"
        elif has_native and not has_docker:
            return "native"
        elif has_docker and has_native:
            # Prefer Docker if both exist (legacy behavior)
            return "docker"
        else:
            return "auto"
