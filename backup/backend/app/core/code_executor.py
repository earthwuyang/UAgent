...
class CodeExecutor:
    ...
    def _detect_deployment_method(self, task_description: str, config: Dict[str, Any]) -> str:
        """Detect the preferred deployment method from task description and config"""

        # Check explicit constraints first
        constraints = config.get('constraints', {})
        if constraints.get('no_docker') or constraints.get('native_compilation'):
            return "native"
        if constraints.get('use_docker') or constraints.get('containerize'):
            return "docker"

        # Analyze task description for deployment hints
        desc_lower = task_description.lower()

        # Native compilation indicators
        native_keywords = [
            'clone', 'compile', 'source code', 'make', 'gcc', 'configure',
            'build from source', 'native', 'install on host', 'system install',
            'git clone', './configure', 'make install', 'natively',
            'compile and run', 'host system', 'direct installation'
        ]

        # Docker/container indicators
        docker_keywords = [
            'docker', 'container', 'dockerfile', 'docker-compose',
            'containerize', 'image', 'orchestrate', 'kubernetes'
        ]

        # Count keyword matches
        native_score = sum(1 for keyword in native_keywords if keyword in desc_lower)
        docker_score = sum(1 for keyword in docker_keywords if keyword in desc_lower)
        ...
        # Default to auto-detect for ambiguous cases
        return "auto"
    ...
    async def _generate_initial_code(self, task_description: str, config: Dict[str, Any], node_id: str = None) -> List[str]:
        ...
        # Detect deployment method from task description and config
        deployment_method = self._detect_deployment_method(task_description, config)

        # Build deployment-specific prompts
        if deployment_method == "native":
            system_prompt = """You are an expert systems programmer and engineer. Generate a COMPLETE native implementation solution that includes:

            1. ALL necessary source code files and build scripts
            2. NATIVE compilation and installation procedures (NO Docker/containers)
            3. Direct host system deployment and configuration
            4. AUTOMATED scripts for building, testing, and deployment on host
            5. VALIDATION scripts to verify the native installation works

            CRITICAL: Use native compilation (make, gcc, configure, etc.) NOT containerization.
            Generate production-ready, complete solutions that run directly on the host system."""

            deployment_files = """
            ## 1. Native Implementation
            - Source code files and build configurations
            - Native build scripts (Makefile, configure scripts, etc.)
            - System service configurations

            ## 2. Native Installation Scripts
            - build.sh: Compile from source using make/gcc
            - install.sh: Install binaries and configs to system paths
            - run.sh: Start services natively on host
            - test.sh: Test native installation

            ## 3. System Integration
            - Service configuration files (systemd, init.d, etc.)
            - Native database initialization scripts
            - Host system path and permission setup"""

        elif deployment_method == "docker":
            system_prompt = """You are an expert DevOps engineer. Generate a COMPLETE containerized deployment solution that includes:

            1. ALL necessary code files and container configurations
            2. DOCKER containerization with Dockerfile and docker-compose.yml
            3. AUTOMATED scripts for building, testing, and deployment
            4. ERROR handling and logging for every step
            5. VALIDATION scripts to verify the containerized deployment works

            CRITICAL: Use Docker containerization for deployment.
            Generate production-ready, complete containerized solutions."""
            ...
            - run.sh: Run containers locally
            - test.sh: Test containerized deployment
            - deploy.sh: Deploy containers to production"""

        else:  # hybrid or auto-detect
            system_prompt = """You are an expert systems engineer. Generate a COMPLETE deployment solution using the MOST APPROPRIATE method for the task:

            1. ALL necessary implementation files
            2. DETAILED step-by-step reproduction instructions
            3. AUTOMATED scripts for building, testing, and deployment
            4. ERROR handling and logging for every step
            5. VALIDATION scripts to verify the deployment works

            CRITICAL: Choose the most appropriate deployment method (native compilation, containers, etc.) based on the task requirements.
            Generate production-ready, complete solutions."""

            deployment_files = """
            ## 1. Implementation Files
            - All necessary source and configuration files
            - Appropriate deployment configurations

            ## 2. Deployment Scripts
            - build.sh: Build the application appropriately
    ...
    async def _generate_comprehensive_solution(self, task_description: str, config: Dict[str, Any], node_id: str = None, supporting_materials: Optional[Dict[str, Any]] = None) -> str:
        ...
        # Build deployment-specific prompts
        if deployment_method == "native":
            system_prompt = """You are an expert systems programmer. Generate a COMPLETE native implementation solution that includes:

            1. ALL necessary source code and build configurations
            2. NATIVE compilation and installation procedures (NO containers)
            3. Direct host system deployment and configuration
            4. AUTOMATED scripts for building, testing, and deployment on host
            5. VALIDATION scripts to verify the native installation works
            ...
            implementation_section = """## 1. Native Implementation
- Source code files and build configurations
- Native build scripts (Makefile, configure scripts, etc.)
- System service configurations"""
            ...
            implementation_section = """## 1. Container Implementation
- All necessary source files
- Complete Dockerfile and docker-compose.yml
- Container configuration files"""
            ...
            implementation_section = """## 1. Implementation Files
- All necessary source and configuration files
- Appropriate deployment configurations"""
...
"""
    ...
    async def _gather_supporting_materials(self, task_description: str, workspace: Path) -> Optional[Dict[str, Any]]:
        try:
            # Create deployment-agnostic search query
            deployment_method = self._detect_deployment_method(task_description, {})

            if deployment_method == "native":
                search_query = f"{task_description} compilation native installation"
            elif deployment_method == "docker":
                search_query = f"{task_description} Docker deployment"
            else:
                search_query = f"{task_description} implementation setup"
