...
class BaseAgent:
    def __init__(self, model="gpt-4o-mini", notes=None, max_steps=100, openai_api_key=None):
        ...
        self.openai_api_key = openai_api_key
        # Support for other API keys from environment variables
    ...
    def get_api_keys_for_model(self):
        """Return appropriate API keys based on the model"""
    ...
    def override_inference(self, query, temp=0.0):
        ...
        api_keys = self.get_api_keys_for_model()
        model_resp = query_model(model_str=self.model, system_prompt=sys_prompt, prompt=query, temp=temp, **api_keys)
        return model_resp
    ...
    def inference(self, research_topic, phase, step, feedback="", temp=None):
        ...
        api_keys = self.get_api_keys_for_model()
        model_resp = query_model(model_str=self.model, system_prompt=sys_prompt, prompt=prompt, temp=temp, **api_keys)
        print("^"*50, phase, "^"*50)
...
class ProfessorAgent(BaseAgent):
    ...
    def generate_readme(self):
        ...
        api_keys = self.get_api_keys_for_model()
        model_resp = query_model(model_str=self.model, system_prompt=sys_prompt, prompt=prompt, **api_keys)
