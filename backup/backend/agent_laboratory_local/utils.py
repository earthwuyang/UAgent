...
def query_qwen3_max_preview(prompt, system, api_key, attempt=0, temperature=0.0):
    try:
        ...
        if estimated_tokens > max_tokens:
            ...
            if available_for_prompt > 0:
                ...
                if len(system) > system_char_limit:
                    system = system[:system_char_limit] + "...[truncated]"
        ...
        print(f"Query qwen3-max-preview error: {e}")
        if attempt >= 10:
            return f"Your attempt to query qwen3-max-preview failed: {e}"
        return query_qwen3_max_preview(prompt, system, api_key, attempt+1, temperature)
