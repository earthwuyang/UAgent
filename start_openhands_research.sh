...
# Navigate to UAgent directory (parent of this script)
...
# Load environment variables from .env in current directory (UAgent)
if [ -f ".env" ]; then
...
# Change to OpenHands subdirectory for running the server
cd OpenHands
...
# Use OPENHANDS_PORT from .env, fallback to 3000
export port="${OPENHANDS_PORT:-${port:-3000}}"
