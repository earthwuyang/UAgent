...
class DeepResearchAdapter(AgentAdapter):
    ...
    async def run(self, task: Task, context: Context) -> AsyncIterator[ResearchEvent]:
        ...
        try:
            ...
            yield PlanEvent(
                ...
                reasoning=f"Using web research approach for task: {task.goal[:100]}. Will search for relevant sources, browse top results, and synthesize findings.",
            )
    ...
    async def _execute_research(
        ...
        # Complete
        logger.info(f"[DEEPRESEARCH] Adapter execution completed for task {task.id}")
        yield CompleteEvent(
            branch_id=context.branch_id,
            node_id=task.id,
