...
class SequentialExperimentPlan:
    ...
    covered_hypotheses: List[str] = field(default_factory=list)
    experiment_hypothesis_map: Dict[str, List[str]] = field(default_factory=dict)
...
class TechnicalGuideProvider:
    ...
    # Domain-specific guides removed to keep prompts generic.
    GUIDES: Dict[str, str] = {}
    ...
    def get_guide(self, task_key: str) -> str:
        ...
        return (
            f"# Technical Guide Requested: {task_key}\n\n"
...
"""
        ...
        experiment_hypothesis_map: Dict[str, List[str]] = {}
        for i, exp_data in enumerate(experiments_data):
            ...
            experiment_hypothesis_map[exp_id] = [hypothesis.id]
        ...
        plan = SequentialExperimentPlan(
            ...
            covered_hypotheses=[hypothesis.id],
            experiment_hypothesis_map=experiment_hypothesis_map,
        )
...
class ExperimentExecutor:
    ...
    def _clean_code_block(raw_code: str) -> str:
        ...
        # Prefer fenced block content when both fences exist
        fenced = re.findall(r"```(?:python|py)?\s*(.*?)```", cleaned, re.DOTALL)
        if fenced:
            cleaned = fenced[0].strip()
        else:
            # Heuristic: strip a leading single fence line if present without closing
            if cleaned.startswith("```"):
                ...
                if parts:
                    parts = parts[1:]
                    cleaned = "\n".join(parts).strip()
            # remove trailing lone ``` if present
            if cleaned.endswith("```"):
...
"""
    ...
    async def execute_sequential_experiments(
        ...
        prior_errors = list(prior_errors or [])
        attempt_context = attempt_context or {}
        max_resume_attempts = max(1, int(self.config.get("max_resume_attempts", 5)))
        ...
        try:
            ...
            if data_result and isinstance(data_result, dict) and (
                ...
                # Pass experiment goal for context-aware validation
                experiment_goal = comprehensive_prompt if comprehensive_prompt else ""
                llm_success, llm_reason = await self._assess_final_json_success(data_result, experiment_goal)
...
═══════════════════════════════════════════════════════════════════════════════
ANTI-SIMULATION REQUIREMENTS (CRITICAL - FAILURE TO COMPLY RESULTS IN REJECTION)
═══════════════════════════════════════════════════════════════════════════════

⛔ ABSOLUTELY FORBIDDEN:
- DO NOT use dummy simulation, mock simulation, or fake experimental operations
- DO NOT generate random/synthetic data (e.g., random.uniform(), fake timings)
- DO NOT create placeholder/simulated results just to complete the task
- DO NOT use phrases like "simulated", "sample data", "demonstration purposes" in analysis
- DO NOT finish the experiment until ALL steps are ACTUALLY completed with REAL data

✅ MANDATORY REQUIREMENTS:
- MUST collect real measurements from actual software execution
- MUST actually deploy and run the complete system end-to-end
- MUST generate real performance data from actual queries/workloads
- MUST train actual ML models with real collected data (not simulated)
- MUST embed trained models into source code (not just design/plan)
- MUST run comprehensive end-to-end tests with measurable results

⚠️ IMPORTANT: If infrastructure limitations prevent real execution:
- DO NOT simulate - instead REPORT the blocker and ASK for help
- DOCUMENT the specific infrastructure limitation preventing completion
- PROPOSE concrete solutions (e.g., "need sudo for initdb", "need XYZ package")
- WAIT for resolution rather than substituting with fake data

The experiment is NOT complete until:
1. Real data has been collected from actual system execution
2. ML model has been trained on real performance measurements
3. Model has been embedded in C/C++ source code
4. Online routing mechanism has been implemented and tested
5. End-to-end performance testing has been conducted with measurable results
...
class ScientificResearchEngine:
    ...
    def __init__(
        ...
        self.max_attempts_per_experiment = max(1, int(self.config.get("max_attempts_per_experiment", 5)))  # Increased to allow proper completion
    ...
    def _build_combined_hypothesis(
        self,
        hypotheses: List[ResearchHypothesis],
    ) -> ResearchHypothesis:
        """Create a meta hypothesis that bundles multiple hypotheses together."""

        combined_id = f"combined_{uuid.uuid4().hex[:8]}"
        statement_lines = [
            "Evaluate the following hypotheses within a SINGLE integrated experiment sequence:",
        ]
        reasoning_lines = ["Rationale for combined evaluation:"]
        predictions: List[str] = []
        success: Dict[str, Any] = {}
        variables: Dict[str, Any] = {}

        for idx, hyp in enumerate(hypotheses, start=1):
            statement_lines.append(f"{idx}. [{hyp.id}] {hyp.statement}")
            reasoning_lines.append(f"{idx}. [{hyp.id}] {hyp.reasoning}")

            hyp_predictions = hyp.testable_predictions or []
            if isinstance(hyp_predictions, (list, tuple)):
                predictions.extend([f"[{hyp.id}] {pred}" for pred in hyp_predictions if pred])
            else:
                predictions.append(f"[{hyp.id}] {hyp_predictions}")

            hyp_success = hyp.success_criteria or {}
            if isinstance(hyp_success, dict):
                for key, value in hyp_success.items():
                    success[f"{hyp.id}:{key}"] = value
            else:
                success[f"{hyp.id}:criterion"] = hyp_success

            hyp_variables = hyp.variables or {}
            if isinstance(hyp_variables, dict):
                for key, value in hyp_variables.items():
                    variables[f"{hyp.id}:{key}"] = value
            else:
                variables[f"{hyp.id}:variable"] = hyp_variables

        if not predictions:
            predictions = ["Each hypothesis above must be validated within the shared experimental run."]
        if not success:
            success = {"combined": "Meet the success criteria for each individual hypothesis."}
        if not variables:
            variables = {"combined": "Use variables defined within individual hypotheses."}
    ...
    async def _assess_final_json_success(
        ...
        Args:
            final_json: The experiment results to validate
            experiment_goal: The original experiment goal/prompt for context
        """
        ...
        try:
            # First, programmatic checks for simulation keywords
            simulation_keywords = [
                "simulated", "simulation", "sample data", "demonstration purposes",
                "placeholder", "mock", "fake", "synthetic data", "random.uniform",
                "for demonstration", "dummy data", "not performed", "design completed",
                "design only", "planned but not", "simulating"
            ]

            final_json_text = json.dumps(final_json, indent=2).lower()

            # Check for simulation keywords
            found_keywords = [kw for kw in simulation_keywords if kw in final_json_text]
            if found_keywords:
                return False, f"Simulation detected: found keywords {found_keywords[:3]}"

            # Check limitations section for red flags
            analysis = final_json.get("analysis", {})
            if isinstance(analysis, dict):
                limitations = analysis.get("limitations", [])
                if isinstance(limitations, list):
                    for limitation in limitations:
                        limitation_lower = str(limitation).lower()
                        if any(kw in limitation_lower for kw in ["simulated", "simulation", "not performed", "demonstration"]):
                            return False, f"Limitation indicates incomplete work: {limitation}"

            # Check if ML model training is mentioned as NOT done
            conclusions = final_json.get("conclusions", [])
            if isinstance(conclusions, list):
                conclusions_text = " ".join(str(c).lower() for c in conclusions)
                if "not performed" in conclusions_text or "not trained" in conclusions_text:
                    return False, "Conclusions indicate ML model training was not performed"

            # Check for required artifacts - ML model files, training data, etc.
            modifications_made = []
            if isinstance(analysis, dict):
                modifications_made = analysis.get("modifications_made", [])

            has_ml_model_embed = False
            has_routing_implementation = False
            if isinstance(modifications_made, list):
                mods_text = " ".join(str(m).lower() for m in modifications_made)
                # Look for evidence of ML model embedding
                if any(phrase in mods_text for phrase in ["embed model", "ml model in c", "model weights", "inference engine"]):
                    has_ml_model_embed = True
                # Look for evidence of routing implementation
                if any(phrase in mods_text for phrase in ["routing mechanism", "query routing", "adaptive routing"]):
                    has_routing_implementation = True

            # For ML-based experiments, require evidence of model deployment
            experiment_goal_lower = str(final_json.get("_experiment_goal", "")).lower()
            if "ml" in experiment_goal_lower or "machine learning" in experiment_goal_lower or "model" in experiment_goal_lower:
                if not has_ml_model_embed:
                    return False, "ML experiment but no evidence of model embedding in source code"
                if "routing" in experiment_goal_lower and not has_routing_implementation:
                    return False, "Routing experiment but no evidence of routing mechanism implementation"

            # Restore original case for LLM assessment
            final_json_text = json.dumps(final_json, indent=2)
            if len(final_json_text) > 6000:
                final_json_text = final_json_text[:6000] + "\n... (truncated)"

            prompt = f"""
You are auditing an experiment report. Decide if the experiment genuinely succeeded with REAL execution and deployment.

❌ REJECT (success=false) if you find ANY of these RED FLAGS:
- Contains words: "simulated", "simulation", "sample data", "demonstration", "placeholder", "mock"
- Training data was generated with random.uniform() or similar fake data generation
- Analysis/limitations mention "data was simulated" or "for demonstration purposes"
- ML model was NOT actually trained (only designed/planned)
- ML model was NOT embedded in C/C++ source code (only proposed)
- System was NOT fully deployed and tested end-to-end
- Performance results are synthetic/estimated rather than measured from real execution
- Critical components are missing (e.g., "ML model training was not performed")

✅ ACCEPT (success=true) ONLY if ALL of these are confirmed:
- Real data collected from actual software execution (not random/synthetic)
- ML model actually trained on real performance measurements
- Trained model weights embedded in C/C++ source code
- Online routing mechanism implemented and tested
- End-to-end system deployment completed successfully
- Performance testing conducted with measurable real-world results

Return STRICT JSON with keys:
- "success": true or false (be VERY strict - when in doubt, mark false)
- "reason": specific explanation citing what is missing or why it succeeded (<=80 words)
...
""".strip()
    ...
    async def _run_experiments_for_idea(
        ...
        while iteration < max_iterations:
            ...
            if session_id:
                parent_id = idea.node_id or next((hyp.node_id for hyp in pending_hypotheses if hyp.node_id), None)
                if not parent_id:
                    parent_id = self._get_root_node_id(session_id)

                iteration_phase = f"{idea.id}_experiments_iter_{iteration}"
                iteration_message = (
                    f"Iteration {iteration} across {len(pending_hypotheses)} hypotheses"
                )
                iteration_metadata = {
                    "parent_id": parent_id,
                    "node_type": "iteration",
                    ...
                    "title": f"Idea iteration {iteration}",
                }
            ...
            combined_hypothesis = self._build_combined_hypothesis(pending_hypotheses)
            experiments_requested = max(1, self.experiments_per_hypothesis)

            self.logger.info(
                "Designing combined sequential plan with %d experiments covering %d hypotheses",
                experiments_requested,
                len(pending_hypotheses),
            )

            sequential_plan = await self.experiment_designer.design_sequential_experiments(
                hypothesis=combined_hypothesis,
                num_experiments=experiments_requested,
                technical_requirements=technical_requirements,
                technical_guide_provider=self.technical_guide_provider,
            )

            sequential_plan.covered_hypotheses = [hyp.id for hyp in pending_hypotheses]
            sequential_plan.experiment_hypothesis_map = {
                design.id: [hyp.id for hyp in pending_hypotheses]
                for design in sequential_plan.experiments
            }
            ...
            if technical_requirements:
                ...
                if not is_valid:
                    self.logger.warning(
                        "Combined experiment plan validation failed with %d errors",
                    ...
                    if session_id and iteration_node_id:
                        await self._log_progress(
                            session_id,
                            phase=f"{idea.id}_combined_iter_{iteration}_validation_failed",
                            progress=iteration_progress + per_iteration_increment * 0.15,
                            message="Combined plan validation failed - redesigning with stricter constraints",
                            metadata={
                                "parent_id": iteration_node_id,
                    ...
                    self.logger.info("Retrying combined experiment design with validation feedback")
                    sequential_plan = await self.experiment_designer.design_sequential_experiments(
                        hypothesis=combined_hypothesis,
                        num_experiments=experiments_requested,
                        technical_requirements=technical_requirements,
            ...
            idea.experiments.extend(sequential_plan.experiments)

            if session_id and iteration_node_id:
                await self._log_progress(
                    session_id,
                    phase=f"{idea.id}_combined_iter_{iteration}_sequential_plan_{sequential_plan.id}",
                    progress=iteration_progress + per_iteration_increment * 0.2,
                    message=f"Designed combined sequential plan: {sequential_plan.overall_objective}",
                    metadata={
                        "parent_id": iteration_node_id,
                        "node_type": "plan",
                        "title": f"Plan: {sequential_plan.overall_objective}",
                        "num_experiments": sequential_plan.num_experiments,
                        "hypothesis_ids": sequential_plan.covered_hypotheses,
                    },
                    parent_phase=f"{idea.id}_experiments_iter_{iteration}",
                )

            combined_executions = await self._execute_sequential_plan_with_retries(
                idea=idea,
                hypothesis_id=sequential_plan.hypothesis_id,
                plan=sequential_plan,
                session_id=session_id,
                iteration_progress=iteration_progress,
                per_iteration_increment=per_iteration_increment,
                iteration_node_id=iteration_node_id,
                session_context=openhands_context,
                iteration_number=iteration,
            )

            idea.executions.extend(combined_executions)

            hypothesis_lookup = {hyp.id: hyp for hyp in idea.hypotheses}
            experiments_per_hypothesis: Dict[str, int] = {}
            for exp_id, hyp_ids in sequential_plan.experiment_hypothesis_map.items():
                for hyp_id in hyp_ids:
                    experiments_per_hypothesis[hyp_id] = experiments_per_hypothesis.get(hyp_id, 0) + 1

            experiment_counter: Dict[str, int] = {}

            for exp_index, execution in enumerate(combined_executions):
                if exp_index >= len(sequential_plan.experiments):
                    break
                design = sequential_plan.experiments[exp_index]
                target_hypothesis_ids = sequential_plan.experiment_hypothesis_map.get(
                    design.id,
                    [design.hypothesis_id],
                )

                attempt_node_id_for_result = (
                    execution.intermediate_results.get("attempt_node_id")
                    if hasattr(execution, "intermediate_results")
                    else None
                )
                attempt_num = (
                    execution.intermediate_results.get("attempt_number", 1)
                    if hasattr(execution, "intermediate_results")
                    else 1
                )

                for target_id in target_hypothesis_ids:
                    target_hypothesis = hypothesis_lookup.get(target_id)
                    if not target_hypothesis:
                        continue

                    exp_index_for_target = experiment_counter.get(target_id, 0) + 1
                    experiment_counter[target_id] = exp_index_for_target
                    total_for_target = experiments_per_hypothesis.get(
                        target_id,
                        sequential_plan.num_experiments,
                    )

                    design_for_target = replace(design, hypothesis_id=target_id)
                    ...
                    if execution.status == ExperimentStatus.COMPLETED:
                        ...
                        await self._update_hypothesis_status(target_hypothesis, experiment_result)

                        if session_id and attempt_node_id_for_result:
                            await self._log_progress(
                                session_id,
                                phase=f"{idea.id}_hyp_{target_id}_iter_{iteration}_attempt_{attempt_num}_result_{exp_index_for_target}",
                                progress=iteration_progress + per_iteration_increment * 0.65,
                                message=f"Result: {design.name} (Exp {exp_index_for_target}/{total_for_target})",
                                metadata={
                                    "parent_id": attempt_node_id_for_result,
                                    "node_type": "result",
                                    "title": f"Result: {design.name}",
                                    "conclusions": experiment_result.conclusions[:2],
                                    "confidence_score": experiment_result.confidence_score,
                                    "experiment_index": exp_index_for_target,
                                    "total_experiments": total_for_target,
                                    "hypothesis_id": target_id,
                                },
                                parent_phase=f"{idea.id}_hyp_{target_id}_iter_{iteration}_attempt_{attempt_num}",
                            )
                    else:
                        target_hypothesis.evidence.append(
                            f"Experiment {exp_index_for_target} ({design.name}) failed"
                        )
                        if session_id and attempt_node_id_for_result:
                            await self._log_progress(
                                session_id,
                                phase=f"{idea.id}_hyp_{target_id}_iter_{iteration}_attempt_{attempt_num}_result_{exp_index_for_target}_failed",
                                progress=iteration_progress + per_iteration_increment * 0.65,
                                message=f"Failed: {design.name} (Exp {exp_index_for_target}/{total_for_target})",
                                metadata={
                                    "parent_id": attempt_node_id_for_result,
                                    "node_type": "result",
            ...
            if iteration < max_iterations and iteration_results:
                await self._refine_hypotheses_for_next_iteration(
                    idea.hypotheses,
                    iteration_results,
                )
    ...
    async def _execute_sequential_plan_with_retries(
        ...
        attempt_base_progress = iteration_progress + per_iteration_increment * 0.4
        parent_phase = f"{idea.id}_hyp_{hypothesis_id}_iter_{iteration_number}"

        for attempt in range(1, self.max_attempts_per_experiment + 1):
            attempt_progress = attempt_base_progress + per_iteration_increment * min(0.15, 0.05 * attempt)
            attempt_phase = f"{idea.id}_hyp_{hypothesis_id}_iter_{iteration_number}_attempt_{attempt}"
            attempt_node_id: Optional[str] = None
